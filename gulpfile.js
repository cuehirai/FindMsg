...
const commitInfo = () => { try { return execSync("git describe --all --long --always --abbrev=40 --dirty --broken").toString().trim(); } catch (e) { return "commit info unknown" } }
const commitNumber = () => { try { return execSync("git rev-list --count HEAD").toString().trim(); } catch (e) { return "commit number unknown" } }
...
const styles = () => {
    return src('src/app/**/*.scss')
        // 2020/07/22 remove plumber and gulp-plumber, since I can see no clear reason for their use and those packages are ancient
        // if necessary, https://www.npmjs.com/package/pump seems to provide similar functionality and is actually maintained
        //.pipe($.plumber())
...
// TASK: nuke
task('nuke', async () => {
    const pDel = del(['temp', 'dist/*', 'dist/web/*', '!dist/web'/*, '!dist/web/.git'*/], {});

    const res = await pDel;
    console.log(res.join('\n'));

    return res; //del(['temp', 'dist'], { dryRun: true, dot: false, ignore: ["web/.git"] });
});
...
task('nukebuild', series('nuke', 'build'));
